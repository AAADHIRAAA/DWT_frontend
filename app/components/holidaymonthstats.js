...
const DialogBox = () => {
    ...
    // const addHoliday = () => {
    //     setHolidays([{ date: getCurrentYearStartDate(), name: "" }, ...holidays]);
    // };
    const addHoliday = () => {
    ...
    //     const response = await fetch(`https://trackserv.techfiz.com/api/v1/admin/monthlyholidays/${selectedYear}`);
    //     if (!response.ok) {
    //         throw new Error(`HTTP error! Status: ${response.status}`);
    //     }
    //     const holidayData = await response.json();
    //     if (holidayData) {
    //         const formattedData = holidayData.flatMap(monthData =>
    //             monthData.holidays.map(item => {
    //                 const date = new Date(item.holidayDate);
    //                 if (isNaN(date.getTime())) {
    //                     console.error(`Invalid date: ${item.holidayDate}`);
    //                     return null;
    //                 }
    //                 return {
    //                     date: date,
    ...
    return (
        <Dialog open={isOpen} onOpenChange={() => setIsOpen(!isOpen)}>
            <DialogTrigger className="text-sky-600">
                <button className={" text-black "}>Edit Holidays</button>
            </DialogTrigger>
                    ...
                    <div className={" overflow-auto max-h-[40vh]"}>
                        <div id={"holidays"}>
                            {holidays.map((holiday, index) => (
                                <div key={index} className={"flex flex-row justify-center align-middle items-center"}>
